...
export const analyzeRoutesFlood = (routes, floodZones) => {
  console.log(
    `🔍 analyzeRoutesFlood: Checking ${routes.length} routes against ${floodZones.length} flood zones`
  );
  console.log(
    "🌊 Flood zones detail:",
    floodZones.map((z) => ({
  ...
  const analyzed = routes.map((route, index) => {
    ...
    console.log(
      `📍 Route ${index + 1}: ${distance.toFixed(2)}km, ${Math.round(
        duration
      )}min → Intersects ${affectedZones.length} flood zones`
    );
    if (affectedZones.length > 0) {
      console.log(
        `   ⚠️ Affected zones:`,
        affectedZones.map((z) => z.name || z.id)
      );
    }
  ...
  console.log(
    `✅ Safe routes (no flood intersections): ${safeRoutes.length}/${analyzed.length}`
  );
  console.log(
    `⚠️ Unsafe routes (have flood intersections): ${
      analyzed.length - safeRoutes.length
    }/${analyzed.length}`
  );
  ...
  if (safeRoutes.length > 0) {
    ...
    safeRoutes.forEach((r, i) => {
      console.log(
        `  ${i + 1}. Original route ${r.index + 1}: ${r.distance.toFixed(
          2
        )}km, ${Math.round(r.duration)}min`
      );
    });
  }
  ...
  if (safeRoutes.length < analyzed.length) {
    ...
    analyzed
      .filter((r) => r.floodCount > 0)
      .forEach((r, i) => {
        console.log(
          `  ${i + 1}. Original route ${r.index + 1}: ${r.distance.toFixed(
            2
          )}km, ${Math.round(r.duration)}min → ${r.floodCount} zones`
        );
      });
  }

  // ✅ Nếu KHÔNG có route an toàn nào, trả về object đặc biệt
  if (safeRoutes.length === 0) {
    console.warn("⚠️ CẢNH BÁO: Tất cả routes đều đi qua vùng ngập!");
    return {
      allUnsafe: true,
      totalRoutes: analyzed.length,
      routes: [], // Không trả về routes nào
      message:
        "Tất cả các tuyến đường đều đi qua vùng ngập lụt. Vui lòng chọn điểm đến khác hoặc đợi thời tiết tốt hơn.",
    };
  }
